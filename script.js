...
function changeToggleButton(themeType) {
  var toggleButton = document.getElementById("hit-toggle");
  if (toggleButton) {
    if (themeType === "light") {
      toggleButton.classList.remove("cs-justify-content-end");
      toggleButton.classList.add("cs-justify-content-start");
    } else if (themeType === "dark") {
      toggleButton.classList.remove("cs-justify-content-start");
      toggleButton.classList.add("cs-justify-content-end");
    }
  }
}

function toggleDataTheme(toggleHit = false) {
  const savedDataTheme = JSON.parse(localStorage.getItem("dataTheme"));
  if (toggleHit === false) {
    document.querySelector("html").setAttribute("data-theme", savedDataTheme);
    return;
  }
  if (savedDataTheme) {
    newDataTheme = savedDataTheme === "light" ? "dark" : "light";
    document.querySelector("html").setAttribute("data-theme", newDataTheme);
    localStorage.setItem("dataTheme", JSON.stringify(newDataTheme));
    changeToggleButton(newDataTheme);
  } else {
    document.querySelector("html").setAttribute("data-theme", "light");
    localStorage.setItem("dataTheme", JSON.stringify("light"));
    changeToggleButton("light");
  }
}
